...
class PastAppointmentTab extends StatefulWidget {
  ...
  @override
  State<PastAppointmentTab> createState() => _PastAppointmentTabState();
}

class _PastAppointmentTabState extends State<PastAppointmentTab> {
  // Controller để phát hiện khi người dùng cuộn đến cuối danh sách
  final ScrollController _scrollController = ScrollController();
  late PastAppointmentCubit _appointmentCubit;
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) {
        _appointmentCubit = PastAppointmentCubit()..getPastAppointments();
        return _appointmentCubit;
      },
      child: BlocBuilder<PastAppointmentCubit, PastAppointmentState>(
        builder: (context, state) {
          if (state is PastAppointmentLoading) {
            return const Center(child: CircularProgressIndicator());
          } else if (state is PastAppointmentError) {
            return Center(child: Text('Error: ${state.message}'));
          } else if (state is PastAppointmentLoaded || state is PastAppointmentLoadingMore) {
            // Lấy appointments từ state
            List<PastAppointmentEntity> appointments = [];
            bool isLoadingMore = false;

            if (state is PastAppointmentLoaded) {
              appointments = state.appointments;
            } else if (state is PastAppointmentLoadingMore) {
              appointments = state.appointments;
              isLoadingMore = true;
            }
